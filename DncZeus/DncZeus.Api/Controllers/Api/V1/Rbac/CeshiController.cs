...
using DncZeus.Api.RequestPayload.Rbac.Ceshi;
using DncZeus.Api.RequestPayload.Rbac.Role;
using DncZeus.Api.Utils;
using DncZeus.Api.ViewModels.Rbac.DncCeshi;
...
namespace DncZeus.Api.Controllers.Api.V1.Rbac
{
    ...
    public class CeshiController : ControllerBase
    {
        ...
        public CeshiController(DncZeusDbContext dbContext, IMapper mapper)
        {
        ...
        /// <returns></returns>
        [HttpPost]
        public IActionResult List(CeshiRequestPayload payload)
        {
            var response = ResponseModelFactory.CreateResultInstance;
            using (_dbContext)
            {
                var query = _dbContext.DncCeshi.AsQueryable();
                if (!string.IsNullOrEmpty(payload.Kw))
                {
                    query = query.Where(x => x.A.Contains(payload.Kw.Trim()) || x.B.Contains(payload.Kw.Trim()));
                }
               /* if (payload.IsDeleted > CommonEnum.IsDeleted.All)
                {
                    query = query.Where(x => x.C == payload.IsDeleted);
                }
                if (payload.Status > CommonEnum.Status.All)
                {
                    query = query.Where(x => x.D == payload.Status);
                }*/
                var list = query.Paged(payload.CurrentPage, payload.PageSize).ToList();
                var totalCount = query.Count();
                var data = list.Select(_mapper.Map<DncCeshi, CeshiCreateViewModel>);
        ...
        public IActionResult Edit(string code)
        {
            using (_dbContext)
            {
                var entity = _dbContext.DncCeshi.FirstOrDefault(x => x.A == code);
                var response = ResponseModelFactory.CreateInstance;
                response.SetData(_mapper.Map<DncCeshi, RoleCreateViewModel>(entity));
                return Ok(response);
            }
        }
